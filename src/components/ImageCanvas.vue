...
// initial number of pixels for the pixalated image
let size = 1
...
onMounted(() => {
  ...
  setSmoothing(false)
  // initiate pixelation when the image is loaded
  image.onload = () => {
...
// imitate progressive loading
function pixelate() {
  // draw smaller version of the image
  context.value.drawImage(image, 0, 0, size, size)

  // stretch the image to the full size
  ...
  // increase size for the next interation
  size *= 2

  // recursivly run next interation with a slight timeout
  setTimeout(
    () => {
      window.requestAnimationFrame(pixelate)
    },
